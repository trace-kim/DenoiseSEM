...
8. Provenance: run the burst arms through the `runctl` bundle machinery
...
The same defect is worse in the older `runctl` (then `ddimctl`) SEM run
...
**The ablation.** `burst_diffusion/configs/miic_p10_dedup_n2n.yml` sets
