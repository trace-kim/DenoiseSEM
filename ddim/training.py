...
This is the ``ddim`` flow's trainer.  It is invoked by ``runctl``'s worker with
an immutable ``RunManifest`` and reuses the flow-agnostic primitives in
:mod:`runctl.control`; the diffusion maths itself still lives in the original
``ddim.models``/``ddim.functions``/``ddim.runners`` modules.
...
``StopController``, ``select_device`` and friends are re-exported so that
...
from runctl.checkpoints import (
    build_checkpoint,
    capture_rng_state,
    load_checkpoint,
    restore_rng_state,
...
from runctl.run_logging import MetricLogger

from ddim.datasets import data_transform, get_dataset, inverse_data_transform
