...
> **Layout note.** This is the original DDIM research code, namespaced as the
> `ddim` package so it can live alongside the other pipelines in this
> repository. Only import paths changed (`models.diffusion` ->
> `ddim.models.diffusion`); the algorithms are untouched.

Start here: [repository workflow guide](../docs/workflows.md), then the
[training quick start / 학습 빠른 시작](../runctl/docs/training_guide.md).

For full operational details, see the reproducible
[`runctl` training workflow](../runctl/docs/training_workflow.md). The original
`main.py` interface below is retained for legacy compatibility and sampling; it
is now run as `python -m ddim.main`.
...
```
python -m ddim.main --config {DATASET}.yml --exp {PROJECT_PATH} --doc {MODEL_NAME} --ni
```

Legacy YAML values can be overridden without editing the config file. Common settings have direct flags:

```text
python -m ddim.main --config sem_ddim_local_32.yml --exp experiments/sem64 --doc sem64-run1 --image-size 64 --batch-size 4 --learning-rate 0.0001 --max-steps 20000 --ni
```
...
```text
python -m ddim.main --config sem_ddim_local_32.yml --exp experiments/custom --doc custom-run --set model.dropout=0.1 --set "model.ch_mult=[1, 2, 2, 4]" --set data.random_flip=true --ni
```
...
```
python -m ddim.main --config {DATASET}.yml --exp {PROJECT_PATH} --doc {MODEL_NAME} --sample --fid --timesteps {STEPS} --eta {ETA} --ni
```
where 
...
```
python -m ddim.main --config {DATASET}.yml --exp {PROJECT_PATH} --use_pretrained --sample --fid --timesteps {STEPS} --eta {ETA} --ni
```
