...
def resolve_config_path(config: str) -> str:
    """Locate a YAML recipe by absolute path, cwd-relative path, or bare name.

    Recipes ship inside the package (``ddim/configs``) so that the legacy
    entry point works from any working directory, while an explicit path still
    wins for out-of-tree experiment configs.
    """
    ...
    if os.path.isabs(config):
        ...
        candidates.append(os.path.join(CONFIG_DIR, config))
        candidates.append(os.path.join("configs", config))
        candidates.append(config)
    for candidate in candidates:
        if os.path.isfile(candidate):
            return candidate
    raise FileNotFoundError(
        "config {!r} not found; looked in {}".format(config, ", ".join(candidates))
    )
...
def parse_args_and_config(
    ...
    if args.is_main_process:
        logging.warning(
            "python -m ddim.main is the legacy interface; use the typed 'runctl' "
            "workflow (runctl train ... --flow ddim) for new SEM runs"
        )
    args.log_path = os.path.join(args.exp, "logs", args.doc)

    # parse config file
    with open(resolve_config_path(args.config), "r", encoding="utf-8") as f:
