...
The worker is flow-agnostic: it verifies the dataset fingerprint, owns the
attempt lifecycle and the process exit-code contract, then hands the manifest
to whichever flow produced it.  Adding a pipeline therefore never requires
touching this file.
"""
...
from .control import StopController
    ...
    try:
        with Heartbeat(state, heartbeat_seconds), _SignalHandlers(stop_controller):
            ...
            with MetricLogger(root / "metrics.jsonl", root / "tensorboard") as metrics:
                ...
                flow = get_flow(str(_field(manifest, "flow", "ddim")))
                _append_line(stdout_path, "using flow {}".format(flow.name))
                result = flow.trainer(
                    manifest,
                    root,
                    resume=resume_path,
