...
def flatten_config(value: Mapping[str, Any], prefix: str = "") -> dict[str, Any]:
    """Flatten a nested YAML mapping into dotted keys.

    Exposed for flow plugins, which map dotted recipe keys onto flat spec field
    names.  Keeping the traversal here means every flow inherits the same
    "keys must be strings" rule.
    """
    ...
    for key, item in value.items():
        ...
        if isinstance(item, Mapping):
            flattened.update(flatten_config(item, dotted))
        else:
...
_flatten_mapping = flatten_config


def load_config_mapping(config_path: Path | str) -> dict[str, Any]:
    """Read a YAML recipe, rejecting duplicate keys and non-mapping documents."""
...
def map_config_keys(
    ...
    *,
    tuple_fields: Collection[str] = (),
    coercions: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Translate dotted recipe keys into flat spec field names.

    Unknown keys and two recipe keys that configure the same spec field are
    both hard errors: a typo must never be silently ignored, and a config must
    never quietly contradict itself.
    """

    unknown = sorted(set(flat) - set(key_map))
    if unknown:
    ...
    for source_key, value in flat.items():
        ...
        if target_key in tuple_fields and isinstance(value, list):
            value = tuple(value)
        coerce = coercions.get(target_key)
...
def coerce_reproducibility(value: Any) -> Any:
    if isinstance(value, str):
        try:
            return ReproducibilityMode(value)
...
def load_training_spec(
    ...
    *,
    ...
    """Resolve one strict base config plus explicit field-name overrides.

    ``flow`` is a :class:`runctl.registry.Flow` (or its name); it supplies both
    the recipe parser and the concrete spec model.  It defaults to ``"ddim"``
    so that existing callers keep working.
    """

    from .registry import Flow, get_flow

    resolved_flow: Flow = flow if isinstance(flow, Flow) else get_flow(flow or "ddim")
    resolved = dict(resolved_flow.parse_config(config_path))
    spec_model = resolved_flow.spec_model
    if overrides:
        unknown = sorted(set(overrides) - set(spec_model.model_fields))
        if unknown:
            raise ConfigurationError("unknown training override(s): " + ", ".join(unknown))
        tuple_fields = {
            name
            for name, info in spec_model.model_fields.items()
            if "tuple" in str(info.annotation)
        }
        for key, value in overrides.items():
            ...
            if key == "reproducibility":
                value = coerce_reproducibility(value)
            resolved[key] = value
    return spec_model.model_validate(resolved)
...
def _source_files(repo_root: Path) -> list[Path]:
    ...
    if git_files is not None:
        ...
        package_roots = ("runctl", "ddim", "burst_diffusion", "noising_pipeline")
        if (repo_root / "runctl").is_dir():
            candidates = []
...
if run_value is None:
    ...
    runpy.run_module("runctl.worker", run_name="__main__")
else:
...
def _run_id(created_at: datetime, spec: BaseTrainingSpec, profile: MachineProfile, argv: tuple[str, ...]) -> str:
    identity = json.dumps(
        {
