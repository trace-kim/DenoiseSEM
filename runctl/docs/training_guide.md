...
2. Press Enter to keep a value from `ddim/configs/sem.yml`, or type a new value.
...
After `runctl run status "$run"` reports `completed`:
...
2. `ddim/configs/sem.yml`의 값을 유지하려면 Enter를 누르고, 변경하려면 새 값을 입력합니다.
...
`runctl run status "$run"`이 `completed`를 표시하면 다음 명령을 실행합니다.
