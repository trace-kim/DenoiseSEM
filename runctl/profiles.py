...
def user_config_dir(
    *,
    ...
    """Return the OS-standard per-user configuration directory for runctl."""
    ...
    if platform_name == "win32":
        ...
        return (Path(base) if base else home_dir / "AppData" / "Roaming") / "runctl"
    if platform_name == "darwin":
        return home_dir / "Library" / "Application Support" / "runctl"
    base = env.get("XDG_CONFIG_HOME")
    return (Path(base) if base else home_dir / ".config") / "runctl"
