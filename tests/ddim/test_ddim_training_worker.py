...
from runctl.checkpoints import build_checkpoint, load_checkpoint, save_checkpoint
from runctl.bundles import fingerprint_dataset
from runctl.run_logging import MetricLogger, StateStore, read_json
from ddim.training import ModernTrainingRunner, StopController, select_device
from runctl.worker import EXIT_USAGE, run_worker, verify_dataset_fingerprint
...
class ModernTrainingTests(unittest.TestCase):
    def test_implicit_device_refuses_accidental_cpu_training(self):
        with mock.patch("runctl.control.torch.cuda.is_available", return_value=False):
    ...
    def test_worker_retains_single_isolated_gpu_invariant(self):
        with mock.patch("runctl.control.torch.cuda.is_available", return_value=True), mock.patch(
            "runctl.control.torch.cuda.device_count", return_value=4
        ):
