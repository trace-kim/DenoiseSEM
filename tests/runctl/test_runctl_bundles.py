...
from runctl.schemas import ExecutorType, MachineProfile, RunStatus
...
def test_run_bundle_is_complete_collision_safe_and_round_trips(tmp_path: Path) -> None:
    ...
    argv = ("python", "-m", "runctl", "train", "launch", "--label", "smoke-test")
    ...
    from runctl.bundles import materialize_source_snapshot
...
def test_worker_bootstrap_isolates_selected_gpu_before_importing_worker(
    ...
    package = run_root / "source" / "runctl"
...
def test_materialization_rejects_parent_traversal_before_extraction(tmp_path: Path) -> None:
    ...
    spec = DdimTrainingSpec(max_steps=1, checkpoint_every=1, validation_every=1, sample_every=1)
