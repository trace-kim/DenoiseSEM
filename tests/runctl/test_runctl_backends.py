...
def test_slurm_doctor_can_validate_resources_without_submitting(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("runctl.backends.shutil.which", lambda command: f"/usr/bin/{command}")
...
def test_external_hpc_prepares_gpu_guarded_worker_and_probe(tmp_path):
    prepared = request(
        ...
        expected_gpu="H100",
        resources={"gpus": 1},
        argv=("/opt/venv/bin/python", "-m", "runctl.worker", "--manifest", "manifest.json"),
    )
    ...
    assert "exec /opt/venv/bin/python -m runctl.worker" in worker
